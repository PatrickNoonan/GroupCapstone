$.ajax({
    ...
    .done(function (data) {
        ...
        let xScale = d3.scaleLinear()
            // input range
            .domain([
                d3.min(monthlyvisits, d => d.month),
                d3.max(monthlyvisits, d => d.month)
            ])
            // output range
            .range([padding + 5, width - padding])

        let yScale = d3.scaleLinear()
            .domain([0, d3.max(monthlyvisits, d => d.visits)])
            .range([height - padding, 10])

        let yAxisGen = d3.axisLeft().scale(yScale).ticks(5)

        let xAxisGen = d3.axisBottom().scale(xScale).tickFormat(d => getMonthFromNum(d))
